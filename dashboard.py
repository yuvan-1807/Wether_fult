...
# ===== ANOMALIES TABLE =====
    ...
    anomalies_df['fault_type'] = anomalies_df['label'].map({
        1: 'Spike Fault',
        2: 'Stuck Sensor',
        3: 'Calibration Drift',
    ...
    st.dataframe(
        anomalies_df[['date', 'temperature', 'humidity', 'rainfall', 'fault_type']],
...
# ===== LIVE DEMO =====
    ...
    st.write("Inject test data and watch the model detect anomalies in real-time!")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.write("### Test Case 1: Spike Fault")
        st.write("Rainfall suddenly jumps to 200mm (extreme spike)")
        
        if st.button("🔴 Inject Spike & Detect"):
            test_spike = pd.DataFrame({
                'temperature': [28.5],
                'humidity': [70],
                'pressure': [1010],
                'rainfall': [200]
    ...
    if st.button("🌪️ Inject Storm Pattern & Detect"):
        test_storm = pd.DataFrame({
            'temperature': [31.5],  # +3°C (warming)
            'humidity': [55],       # -15% (drying)
            'pressure': [1005],     # -5 hPa (pressure drop)
            'rainfall': [5]
        })
        ...
        if pred[0] == 1:
            ...
            st.write("**System correctly identified this as a genuine storm pattern**")
            st.write("All variables correlate → Real event → Pass to duty officer for warning")
        else:
            st.warning("⚠️ Flagged as anomaly - might need cross-validation check")

# ===== FOOTER =====
st.divider()
st.markdown("""
---
**AWS Anomaly Detection System** | Internal Round Demo
- Layer 1: Statistical rules (spikes, stuck sensors)
- Layer 2: Isolation Forest ML (multivariate anomalies)
